...
Work is creating graph elements — nodes, edges, expressions. There is no session container. The loop is: find what's red, read the projection, do the work (create nodes, edges, expressions), pull the andon cord if stuck, watch the graph turn green.
...
2. **Core graph writes.** `createIntent`, `recordExpression`, `createGap`, `createDecision`, `createEdge`. Verify DB state after each call.
...
All test data must use a consistent prefix (e.g., `test-`) for IDs so it can be cleaned up reliably. This applies to **all** ID-generating operations — intents, edges, expressions. Operations that generate dynamic IDs (like `transduceExternal` which creates `transduction-{timestamp}`) must accept an optional ID parameter in tests so cleanup works.

Cleanup in beforeEach/afterEach must respect FK constraint ordering:

```
1. expressions (references nodes)
2. edges (references nodes)
...
```json
{
  ...
  "test": {
    "condition": "string -- REQUIRED for all intent types except gap, signal, and compose. The verifiable claim: what must be true for this intent to be satisfied. Gap nodes have no test condition (the test is not yet articulable). Signal nodes have no test condition (the event already happened -- there is nothing to verify). Compose nodes have a structural test: all contains children are satisfied.",
...
When the blocker is resolved, create a decision node recording what was chosen, what alternatives were considered, and what scope is governed. A `closes` edge from the decision to the gap marks the resolution. Gaps are detected blockers. Decisions are authored closures.
...
1. **Create schema and tables.** Build the `gdd` schema and all Layer 0 tables (`gdd.nodes`, `gdd.edges`, `gdd.expressions`, `gdd.agents`, `gdd.skills`, `gdd.llm_providers`) plus enums. This is raw DDL.
...
4. **Implement operations, record expressions.** Work through the layers. As each operation is implemented and its test passes, record an expression in `gdd.expressions` linking the intent to its artifacts. The intent turns green.

5. **Proceed layer by layer.** Layers are thematic groupings, not a strict build sequence. Dependency order is defined by `blocked-by` edges, not by layer number. A Layer 6 intent blocked by a Layer 7 intent means the Layer 7 work comes first — follow the edges, not the numbering.
...
3. **Work.** Create nodes, edges, and expressions. Record the expression on the intent you satisfied. If the work produced source artifacts, commit and push.
